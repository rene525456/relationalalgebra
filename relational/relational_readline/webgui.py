...
def help(command):
    ...
    if len(p) == 1:
        out += 'HELP command'
        out += 'To execute a query:\n[relation =] query\nIf the 1st part is omitted, the result will be stored in the relation last_.'
        out += 'To prevent from printing the relation, append a ; to the end of the query.'
        out += 'To insert relational operators, type _OPNAME, they will be internally replaced with the correct symbol.'
        out += 'Rember: the tab key is enabled and can be very helpful if you can\'t remember something.'
        return out

    cmd = p[1]

    if cmd == 'QUIT':
        out += 'Quits the program'
    elif cmd == 'LIST':
        out += "Lists the relations loaded"
    elif cmd == 'LOAD':
        out += "LOAD filename [relationame]"
        out += "Loads a relation into memory"
    elif cmd == 'UNLOAD':
        out += "UNLOAD relationame"
        out += "Unloads a relation from memory"
    elif cmd == 'SAVE':
        out += "SAVE filename relationame"
        out += "Saves a relation in a file"
    elif cmd == 'HELP':
        out += "Prints the help on a command"
    elif cmd == 'SURVEY':
        out += "Fill and send a survey"
    else:
...
def mi_exec_line(command):
    ...
    if command.startswith(';'):
        ...
        if len(pars) > 2:
            ...
            defname = None
        return load_relation(filename, defname)

    elif command.startswith('UNLOAD '):
        pars = command.split(' ')
        if len(pars) < 2:
            out = colored("Falta un parametro", 'red')
            return out
        if pars[1] in relations:
            del relations[pars[1]]
            completer.remove_completion(pars[1])
        else:
            out = colored("No existe relacion %s" % pars[1], 'red')
            return out
        pass
    elif command.startswith('SAVE '):
        pars = command.split(' ')
        if len(pars) != 3:
            out = colored("Parametro faltante", 'red')
            return out
        ...
        if defname not in relations:
            out = colored("No existe relacion %s" % defname, 'red')
            return out
